...
| Formularios | Los 18 tipos de activo con banco de preguntas redactado | **1 de 18.** `FRM_Preguntas` tiene 15 filas, todas de `FRM_SOS` |
...
| Datos | Toda la base poblada con datos de prueba realistas | **Hecho.** 34 de fixture y 355 sintéticos en la plantilla |
...
| D-09 | Tipos priorizados | **Se construyen los 18.** Se redactan a partir de los tres existentes y de la práctica de mantenimiento; el funcional corrige sobre texto concreto | **ABIERTA.** 1 de 18 con preguntas |
...
|---|---|---|
| D-02 | **Radio por familia de activo** en `TIP_TiposActivo.RadioGeofencingKm`, no un número único: **0,05 km en 12 tipos** —postes SOS, cámaras, gálibos, video wall y equipos de TI—, **0,1 km en 5** —PMV fijos y móviles, generador, báscula y subestación— y **1,5 km en la fibra**. `RG-01` desreferencia esa columna a través de la orden y del activo | Volcar la columna de `TIP_TiposActivo` en `BD/Modelo_Datos_PLANTILLA.xlsx` |
...
| D-09 | Los 17 bancos de preguntas que faltan | Operación |
