...
|---|---|---|
| Modelo de datos | Referencias cableadas de extremo a extremo, sin tablas huérfanas ni duplicadas | **Declarado.** 28 tablas, 211 columnas, 39 referencias, 23 reglas, `validar_modelo.py` en 0 errores. **Sin cablear en la aplicación** |
...
| D-04 | GPS deficiente | Cierre con excepción: motivo escrito, fotografía, marca y aviso al supervisor. Umbral 50 m | **CERRADA con un mecanismo distinto del propuesto, con 40 m y no 50.** El automático (`RG-19`) nunca funcionó —`USERLOCATIONACCURACY()` no existe en AppSheet, `Precision_GPS` nunca se poblaba—; `ESPEC-004`/`ORDEN-004` lo sustituyó por una marca del técnico, sin fotografía ni aviso automático al supervisor |
...
| D-06 | Ciclo de la OT | Siete estados. El técnico llega hasta En revisión; solo el supervisor cierra o suspende. Vencida al día siguiente. Se permite ejecutar sin orden previa como novedad | **CERRADA en la estructura, sin imponer.** Falta el rechazo. **`Vencida` ya no es automática** (`ESPEC-006`/`ORDEN-006`): el bot que la habría movido "al día siguiente" (`RG-08`) nunca corrió —los bots programados no se ejecutan en la cuenta gratuita— y además dejaba la orden en un estado terminal sin salida para el técnico. Hoy `Vencida` es una decisión del supervisor; `RG-37` (columna virtual `EstaVencida`) solo informa, no mueve el estado |
...
| D-04 | `MAN_Mantenimientos.CierreConExcepcion` y `MotivoExcepcion`, con `RG-03`. **`RG-19` se retiró** (`ESPEC-004`/`ORDEN-004`): calculaba `CierreConExcepcion` con `USERLOCATIONACCURACY()`, que no existe en AppSheet, así que la columna nunca se poblaba y la excepción nunca se disparaba. Hoy `CierreConExcepcion` es una casilla que marca el propio técnico; `UMBRAL_GPS` sigue vivo en `PAR_Parametros`, como referencia para su juicio, no como comparación automática. **Vale 40 m, no 50**: la precisión típica de un móvil a cielo abierto es de 4,9 m, así que 40 deja unas ocho veces de margen | La fila `UMBRAL_GPS` de `PAR_Parametros`; `scripts/modelo_objetivo.py` `REGLAS` sin `RG-02`/`RG-19` |
...
escrita y poblada en las siete filas, y **ninguna de las 23 reglas del modelo la lee**. Hasta que
