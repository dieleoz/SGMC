...
│                                   ESPEC-007 (retira FOT_Fotografias.PrecisionGPS, sin cadena de
│                                   reglas detrás; especificada, no aplicada), ESPEC-008 (protege
│                                   FOT_Fotografias.Ubicacion_LatLong con RG-39, hermana de RG-20;
│                                   encontró y documentó un generador roto; APROBADA CON RIESGOS
│                                   ACEPTADOS 2026-08-11) y ESPEC-009 (protege TecnicoID/UsuarioID/
│                                   UsuarioRegistro/Usuario con RG-41 a RG-44; distingue
│                                   MAN_Mantenimientos.TecnicoID de OT_OrdenesTrabajo.TecnicoID;
│                                   última especificación prevista de esta tanda; especificada, sin
│                                   pasar por el arquitecto)
│
...
| Documento | Contenido | Vigencia |
|---|---|---|
| [docs/ARQUITECTURA_OBJETIVO_SGMC.md](docs/ARQUITECTURA_OBJETIVO_SGMC.md) | Modelo objetivo: 28 tablas, 210 columnas, 39 referencias, 21 reglas | Vigente. **Se regenera, no se edita** |
...
| [docs/sdd/ESPEC-004-cierre-excepcion-manual.md](docs/sdd/ESPEC-004-cierre-excepcion-manual.md) | `RG-02`/`RG-19` dependían de `USERLOCATIONACCURACY()`, que no existe en AppSheet: `CierreConExcepcion` nunca se disparaba. Corrige a que el técnico la marque | **APROBADA CON RIESGOS ACEPTADOS** el 2026-08-11, tercera versión, bajo la vara de `CLAUDE.md` §7.18. Las cuatro condiciones del arquitecto ya están aplicadas (commit `e8befce`), y su único bloqueante se cerró **midiéndolo**: `CierreConExcepcion` ya era `Yes/No` ([ACTA-004](docs/sdd/ACTA-004-lecturas-editor.md)). **APLICADA AL MODELO por [`ORDEN-004`](docs/sdd/ORDEN-004-cierre-excepcion-manual.md)** el mismo día: `RG-02` y `RG-19` se retiraron —ya no existen—, `Precision_GPS` se retiró del modelo. **Pendiente el editor**: quitar la `App formula` de `CierreConExcepcion` y ejecutar `PRUEBA-004` `P-40` a `P-45` |
...
| [docs/sdd/ESPEC-007-precision-gps-fotografias.md](docs/sdd/ESPEC-007-precision-gps-fotografias.md) y [PRUEBA-007](docs/sdd/PRUEBA-007-precision-gps-fotografias.md) | `FOT_Fotografias.PrecisionGPS` usa `USERLOCATIONACCURACY()`, que tampoco existe en AppSheet, pero sin la cadena de reglas de `ESPEC-004`: ninguna regla la lee. Retira la columna; un solo archivo cambia (`scripts/modelo_objetivo.py`), los tres documentos que citaban la fórmula se regeneran solos | Especificada, **no aplicada**. Predicciones verificadas sobre copia temporal (`P-63` a `P-67`). Pendiente confirmar en el editor cuál de las dos ramas de retiro aplica (§2.7, §7) |
| [docs/sdd/ESPEC-008-proteger-ubicacion-fotografias.md](docs/sdd/ESPEC-008-proteger-ubicacion-fotografias.md) y [PRUEBA-008](docs/sdd/PRUEBA-008-proteger-ubicacion-fotografias.md) | `FOT_Fotografias.Ubicacion_LatLong` tiene `HERE()` como `Initial value` y ninguna regla `Editable_If`: el pin de la coordenada de cada fotografía se arrastra a mano y la evidencia queda donde el técnico quiera, sin que nada lo impida. Extiende el mecanismo de `RG-20` con una regla hermana, `RG-39`, y encontró en el camino un generador (`generar_prompt_cableado.py`) que deja de instruir el `Initial value` de una columna en cuanto cualquier regla la toca —defecto preexistente, ya vivo hoy sobre `Coordenadas_Cierre_LatLong`— con el parche exacto verificado | **APROBADA CON RIESGOS ACEPTADOS** el 2026-08-11 (§8), ocho condiciones aplicadas, ampliada a `NOV_Novedades` con `RG-40`. **APLICADA por [`ORDEN-008`](docs/sdd/ORDEN-008-proteger-ubicacion-fotografias.md)**, que de paso corrigió el defecto de generador del §2.6 |
| [docs/sdd/ESPEC-009-proteger-identidad.md](docs/sdd/ESPEC-009-proteger-identidad.md) y [PRUEBA-009](docs/sdd/PRUEBA-009-proteger-identidad.md) | Cuatro columnas resuelven quién hizo algo con `USEREMAIL()`/`LOOKUP(USEREMAIL(),...)` y ninguna tenía `Editable_If`: `MAN_Mantenimientos.TecnicoID`, `NOV_Novedades.UsuarioID` (`Ref`, un toque), `MAN_Mantenimientos.UsuarioRegistro` y `FOT_Fotografias.Usuario` (`Text`). A diferencia de `ESPEC-008`, sí hay una decisión de diseño: verifica y distingue `MAN_Mantenimientos.TecnicoID` (ejecución, autoatribuida) de `OT_OrdenesTrabajo.TecnicoID` (asignación, columna distinta, fuera de alcance) para que congelar la primera no bloquee la reasignación de un supervisor. El generador de cableado ya no necesita parche: `ORDEN-008` lo corrigió | **Última especificación prevista de esta tanda. Sin dictamen todavía** (§8). Predicciones verificadas sobre copia temporal (`P-76` a `P-83`). Pendiente confirmar en el editor si las cuatro columnas ya traen `Initial value`, antes de aplicar `Editable_If` — crítico en `TecnicoID`/`UsuarioID`, que son obligatorias, y con precedente medido: `ACTA-011` encontró 3 de 3 columnas vacías en el editor ese mismo día, una bloqueando producción (§2.9, §6) |
...
| [docs/sdd/ESPEC-006-reemplazo-bots-programados.md](docs/sdd/ESPEC-006-reemplazo-bots-programados.md) | `RG-08` y `RG-12` eran bots programados y no corren en la cuenta gratuita; además `RG-08` movería la orden al estado `Vencida`, que es final, e impediría al técnico tardío cerrarla. Propone `RG-37`, columna virtual `EstaVencida` sobre `OT_OrdenesTrabajo`, y `RG-38`, una vista más una acción (`Data: add a new row...`) sobre `PLA_PlanMantenimiento` que el supervisor pulsa | **CERRADA el 2026-08-11**, en su tercera pasada de arquitecto, con **cuatro riesgos aceptados** con nombre y fecha (§8). **APLICADA AL MODELO por [`ORDEN-006`](docs/sdd/ORDEN-006-reemplazo-bots-programados.md)** el mismo día, tras `ORDEN-004`: `RG-08` y `RG-12` se retiraron —ya no existen—, `RG-37` y `RG-38` las reemplazan. **Pendiente el editor**: cablear `RG-37`/`RG-38` y aplicar `QuienCambia = Supervisor` en producción |
