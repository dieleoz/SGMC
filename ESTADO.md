...
| **Modo offline** | **Activado** (`Settings > Offline mode`) el 2026-08-12 | Nada. Hecho |
| Los **3 bots por evento** (`RG-06`, `RG-07`, `RG-10`) | **Acción de RG-10 a medias.** Se creó `Crear orden de seguimiento Action - 1` pero sin mapeo, lo que causó el error *"does not define a column to set"* y dejó la app móvil en *"is not runnable"*. `Automation > Bots` sigue pendiente de completarse | **Corregir el mapeo de RG-10**: `OTOrigenID` = `[OTID]`, `EstadoOrdenID` = `"Programada"`, `Tipo` = `"Correctivo"`. Después probar las coordenadas (Pruebas A y B de `LO_QUE_SE_HACE_A_MANO.md` Paso 2) |
...
| **`EST_Activo.GeneraAlerta` está VACÍA en producción**, y con ella la condición de `RG-06` nunca es cierta | La plantilla local trae `TRUE` en `Fuera de servicio` y `FALSE` en las otras tres; **producción las tiene vacías las cuatro**. `RG-06` compara `[EstadoActivoID].[GeneraAlerta] = TRUE`, así que **el aviso al CCO cuando un activo queda fuera de servicio no se dispara jamás**. Tercera capa del mismo bot: no existía, no tiene destinatarios, y su condición es imposible | Decidir cómo se repone — va con la decisión de abajo, mismo archivo |
| **`EOT_EstadosOrden.QuienCambia` sigue en `Sistema` en producción** | `ORDEN-006` lo cambió a `Supervisor` para `Programada` y `Vencida` **solo en la plantilla del repositorio**. Son 2 celdas | Lo mismo |
| **`USR-012` existe en producción y NO en la plantilla local** | Se añadió por API el 2026-08-11 para que `LOOKUP(USEREMAIL(), "USR_Usuarios", ...)` dejara de devolver vacío. Producción tiene **12** usuarios y `BD/Modelo_Datos_PLANTILLA.xlsx` genera **11**. **Regenerar la plantilla y subirla borraría `USR-012`**, y con ella vuelve el bloqueo: dos columnas obligatorias se quedan vacías y no se puede guardar ni un mantenimiento ni una novedad | Decidir si la cuenta de pruebas entra en el catálogo del repositorio o se repone a mano tras cada regeneración |
| **`USR_Usuarios.Telefono` es obligatoria en el editor y opcional en el modelo** | Lo dijo la API al rechazar la fila: `Missing value in column: Telefono`. Es la **cuarta divergencia modelo↔editor** del día y la única que se descubrió sin mirar el editor. No se corrige a ciegas: puede que el editor tenga razón, o que la exigencia venga de un `Required_If` que no está en `REGLAS` | Mirar `Data > Columns > USR_Usuarios > Telefono > Require?` |
