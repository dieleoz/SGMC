...
w("")
w("> **Este paso es solo para la hoja de produccion heredada, `Modelo_Datos_09082026`.** Sobre")
w("> `BD/Modelo_Datos_PLANTILLA.xlsx` estas tres columnas **no existen**, asi que no hay nada que")
w("> deshacer: saltese el paso entero. Lo mismo vale para las marcas `OCULTAR` del anexo.")
w("")
...
w("esta vacia en los 18 tipos de la hoja de produccion `Modelo_Datos_09082026`: la version por tipo")
w("comparia contra una celda en blanco y **rechazaria tambien los cierres legitimos**.")
w("")
w("**Si despliega contra `BD/Modelo_Datos_PLANTILLA.xlsx`, es al reves.** Alli la columna esta")
w("poblada en sus 27 tipos, y entonces lo correcto es la variante por tipo:")
w("`... <= [OTID].[ActivoID].[TipoActivoID].[RadioGeofencingKm]`. Compruebe contra que hoja esta")
w("trabajando antes de pegar la expresion.")
w("")
...
w("`scripts/modelo_objetivo.py`. Por eso aqui no hay ficha columna por columna como en los pasos")
w("anteriores, y por eso lo que decida en este paso es lo unico que no queda escrito en ninguna parte.")
w("")
...
w("")
w("> ## Antes de usar este anexo: contra que hoja esta escrito")
w(">")
w("> **Las marcas `OCULTAR` describen la hoja de produccion heredada, `Modelo_Datos_09082026`, y")
w("> solo esa.** Son las columnas que ese libro arrastra y el modelo no declara.")
w(">")
w("> **Sobre `BD/Modelo_Datos_PLANTILLA.xlsx` esas columnas no existen**, asi que ahi no hay nada que")
w("> ocultar y ninguna marca `OCULTAR` de este anexo aplica. La plantilla se genera del modelo con")
w("> `python scripts/generar_plantilla.py` y sale con las columnas que el modelo declara, ni una mas.")
w("> Se comprueba con la regla `F-19`:")
w(">")
w("> ```bash")
...
w(">")
w("> ```")
w("> ok Hoja limpia: ninguna de las 43 columnas retiradas existe ya. No hay nada que ocultar")
w("> ```")
w(">")
w("> **Lo mismo vale para las tres `TRAMPA` del paso 6:** son columnas de la hoja heredada, y sobre la")
w("> plantilla no estan, asi que AppSheet no tiene nada que convertir solo.")
w(">")
w("> **Y ocultarlas ya no es trabajo de nadie.** La decision del 2026-08-09 fue migrar a la hoja")
w("> limpia; el procedimiento esta en `MIGRACION_HOJA_LIMPIA.md`. Este anexo se conserva porque es la")
w("> lista por nombre que esa migracion necesita para borrarlas, **no para esconderlas**.")
w("")
w("Leyenda:")
w("")
