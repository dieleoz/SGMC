...
w("")
w("**No editables** — en `MAN_Mantenimientos`, `Editable_If = FALSE` en las tres columnas de")
w("captura (no cuatro: desde ESPEC-004/ORDEN-004 `Precision_GPS` se retiro del modelo):")
w("")
w("```")
w("Coordenadas_Cierre_LatLong · UbicacionEscaneo_LatLong · FechaHoraEscaneo")
w("```")
w("")
...
w("")
w("`RG-19`, el `App formula` que calculaba esta columna con `Precision_GPS`, se retiro con")
w("ESPEC-004/ORDEN-004 (`USERLOCATIONACCURACY()` no existe en AppSheet, la columna nunca se")
w("poblaba). Hoy `CierreConExcepcion` es una casilla `Yes/No` **libre**, sin `App formula`, que")
w("marca el propio tecnico:")
w("")
w("```")
w("Type:          Yes/No")
w("App formula:   (vacio)")
w('Description:   "¿La app no alcanzó buena precisión al capturar la posición de cierre? Marque')
w('               si es así." (a mano; ningun generador emite Description — ESPEC-004 2.13)')
w("```")
w("")
w("**Confirme el `Type` antes de dar por buena esta tabla.** Si `CierreConExcepcion` quedo `Text`")
w("(el escenario que documenta `S-30`), `RG-03` —que compara contra el booleano `TRUE`— deja de")
w("pedir el motivo aunque la casilla se marque, sin ningun mensaje de error (`ESPEC-004` §2.7).")
w("")
w("`PAR_Parametros.UMBRAL_GPS` se conserva sin lector automatico: es la referencia que el tecnico")
w("consulta para decidir si marca la casilla, no una comparacion que corra sola.")
w("")
