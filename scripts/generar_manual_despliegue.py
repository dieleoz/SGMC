...
w("> una columna se parece al de otra tabla, y nuestras tablas llevan prefijo:")
w("> `UNF_UnidadesFuncionales` no se parece a `UnidadFuncional`, asi que el parecido se rompe.")
w("> Es el precio de la")
...
w("> widgets propios, asi que se pueden asignar de forma determinista en vez de a base de clics. **La")
w("> senal de que la aplicacion recogio el cambio es que el boton `SAVE` pasa de gris a azul**; si")
w("> sigue gris, el cambio se perdera al recargar. Metodo y riesgos en")
...
w("> Las **368** se **derivan del PK sobre el trazado del corredor** en cada pasada del generador:")
w("> son todas distintas y todas están sobre la vía, pero **ninguna está medida**. Con los radios")
w("> por tipo —0,05 km en la mayoría— la aplicación")
