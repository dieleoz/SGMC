...
w("")
w("**El modelo describe datos, no interfaz.** No hay aquí vistas, acciones ni slices: `VISTAS`,")
w("`ACCIONES` y `SLICES` no existen en `modelo_objetivo.py`. Lo único que AppSheet construye solo son")
w("las columnas virtuales `Related`, que salen de las referencias. Las pantallas no, y por eso el")
w("paso de vistas de los manuales generados **declara que no está especificado y pide anotar lo que")
w("se haga**, en vez de decir «se construye sola» — que es la clase de instrucción que este proyecto")
w("tiene prohibida.")
w("")
